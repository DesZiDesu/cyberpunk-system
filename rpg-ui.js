...
globalThis.CyberpunkSystemsFactory = api => {
  ...
  function state(){
    ...
    s.payments??=[];s.properties??=[];s.bd??={enabled:false,status:'stopped',x:.85,y:.45};s.bd.rendering=bdGenerating&&bdGenerationState===s.bd;
  ...
  const displayName=name=>['user','player','USER',api.context()?.name1].includes(name)?api.context()?.name1||'V':api.findEffectiveNpc?.(C.handle(name))?.name||C.text(name,180);
  ...
  function openData(v){
    ...
    popup=dialog(v.title,`<article class="cps-data-reader"><header><small>${E(v.kind||'data')} / ARCHIVE</small><h3>${E(v.title)}</h3><code>${E(v.id||'')}</code></header>${note(displayText(v.description))}${rows.length?`<dl>${rows.map(([k,val])=>`<div><dt>${E(k)}</dt><dd>${E(displayText(val))}</dd></div>`).join('')}</dl>`:''}${v.content?`<section class="cps-rpg-detail">${E(v.content)}</section>`:''}${(v.sections||[]).map(x=>`<section><h4>${E(x.heading)}</h4><div class="cps-rpg-detail">${E(x.body)}</div></section>`).join('')}${!v.content&&!v.sections?.length&&!rows.length?note(tr('Only the supplied summary is available. Full file contents have not been received.','มีเพียงบทสรุปที่ส่งมา ยังไม่ได้รับเนื้อหาไฟล์ฉบับเต็ม')):''}</article>`);
  }
  ...
  function dialog(title,body,cls=''){
    ...
    d.innerHTML=`<header class="cps-rpg-top"><span class="cps-eyebrow">NEURAL INTERFACE / v${E(api.version || '3.1.1')}</span><h2>${E(title)}</h2>${buttons('×','close','aria-label="Close"')}</header><div class="cps-rpg-content">${body}</div>`;
  ...
  function payments(){
    ...
    experience.querySelectorAll('[data-payment]').forEach(el=>{const p=s.payments.find(x=>x.id===el.dataset.payment);
      ...
      el.querySelector('[data-rpg=accept]').onclick=()=>{try{
        ...
        const merchant=actor(p.merchant),id='payment:'+p.id;
        ...
        if(p.property&&!s.properties.some(x=>x.paymentId===p.id))s.properties.push({id:C.uid(),paymentId:p.id,name:p.property,description:p.description,owner:displayName('user'),at:new Date().toISOString()});
  ...
  function requestPayment(data,id){
    ...
    const npc=api.findEffectiveNpc(C.handle(data.merchant));if(!npc)throw Error('Payment sender must be a stored NPC');
    ...
    s.payments.push({id,merchant:npc.name,amount,title:C.text(data.title,180),description:C.text(data.description,5000),items,property:kind==='property'?C.text(data.property||data.title,300):'',status:'pending'});
    event('payment requested',npc.name+': '+data.title+' €$'+amount+'; awaiting player decision');
    payments();
  }
  ...
  function quickhack(name){
    ...
    experience.querySelectorAll('[data-slot]').forEach(b=>b.onclick=()=>guard(()=>{
      ...
      a.ram-=it.ramCost??2;it.cooldownUntil=s.turn+Math.max(1,it.cooldown);
...
function open(name='user',nextTab='status',returnRoute=null){if(nextTab==='mail'){mail.open();return;}guard(()=>actor(name));if(name!=='user'&&!api.findEffectiveNpc(C.handle(name)))return;const previous=panel?.isConnected?{name:actorName,tab,back:panel.cpsBack}:null;actorName=name;tab=nextTab;closePanel();panel=dialog(name==='user'?'Cyberware':`${name} / Cyberware`,'','cps-rpg-main');panel.cpsBack=returnRoute||(previous?()=>open(previous.name,previous.tab,previous.back):null);panel.querySelector('[data-rpg="close"]').onclick=closePanel;panel.addEventListener('cancel',closePanel);render();}
  function render(){
    ...
    if(tab==='quickhacks')area.innerHTML=`${deckMarkup(a)}${heading('OWNED PROGRAMS',tr('Quickhack inventory','คลัง Quickhack'),String(a.inventory.filter(it=>it.category==='quickhack'&&it.quantity>0).length))}<div class="cps-rpg-items">${a.inventory.filter(it=>it.category==='quickhack'&&it.quantity>0).map(it=>itemMarkup(it,s)).join('')}</div>`;
    ...
    body.querySelector('[data-section-picker]').onchange=e=>{if(e.target.value==='mail'){e.target.value=tab;mail.open();return;}navigateSection(e.target.value);};
  ...
  function catalog(slot=''){const target=actorName;const d=dialog('Equipment database',`<label>${tr('Search names or game IDs','ค้นชื่อหรือรหัสไอเท็ม')}<input type="search" data-catalog-search placeholder="Monowire / Items…"></label><label><input type="checkbox" data-technical> ${tr('Include 3,420 technical IDs (unverified display names)','รวมรหัสเทคนิค 3,420 รายการ (ยังไม่ยืนยันชื่อแสดงผล)')}</label>${note(tr('58 named entries plus an imported ID index. Not a verified complete game catalog. SVG symbols are original; role-play stats are editable. Adding an item records a manual grant, not a purchase.','มี 58 รายการชื่ออ่านง่ายและดัชนีรหัสนำเข้า ยังไม่ใช่รายการทุกอย่างที่ตรวจสอบครบ ไอคอน SVG วาดใหม่ ค่าสำหรับโรลเพลย์แก้ไขได้ การเพิ่มเป็นการตั้งอุปกรณ์เอง ไม่ใช่การซื้อ'))}<p data-count></p><div class="cps-rpg-catalog"></div>`);
    const draw=()=>{const q=d.querySelector('input[type=search]').value.trim().toLowerCase(),technical=d.querySelector('[data-technical]').checked;
      const all=[...DB.curated,...(technical?DB.records:[])].filter(x=>(!slot||(x.category==='cyberware'&&C.implantSlot(x.slot)===slot))&&`${x.name} ${x.id} ${x.category}`.toLowerCase().includes(q));d.querySelector('[data-count]').textContent=`${all.length} results · showing ${Math.min(60,all.length)}`;
  ...
  function action(command){const [kind,...rest]=command.split(':'),value=rest.join(':'),a=actor(actorName),s=state();
    if(kind==='tab'){if(value==='mail'){mail.open();return;}navigateSection(value);return;}
    ...
    if(actions[kind])actions[kind]();else if(kind==='slot')catalog(value);else if(kind==='socket-unlock'){C.patchActor(a,{implantUnlocks:{[value]:!a.implantUnlocks?.[value]}});event('implant sockets',`${actorName}: ${value}`);}else if(kind==='equip'){C.equip(a,value);event('equipment',`${actorName}: equipment changed`);}else if(kind==='use'){const it=C.use(a,value,s.turn);event('item used',`${actorName}: ${it.name}`);localSkill(actorName,it.name,it.effect,it);notify('ABILITY / '+it.name,it.effect||`${actorName} activated ${it.name}`);}else if(kind==='edit-item')editItem(a.inventory.find(x=>x.id===value));
    ...
    else if(kind==='objective'){const parts=value.split(':'),index=Number(parts.pop()),q=s.quests.find(x=>x.id===decodeURIComponent(parts.join(':')));if(q&&!q.paid&&q.objectives?.[index]){q.objectives[index].done=!q.objectives[index].done;event('objective',q.title+': '+q.objectives[index].text);}}
    else if(kind==='train'){C.train(a,value);event('training',displayName(actorName)+': '+value);}
    else if(kind==='quest'){const q=s.quests.find(x=>x.id===value);if(q)updateQuest({...q,questId:q.id,status:{active:'completed',completed:'failed',failed:'active'}[q.status]||'active'});}
...
- Mission updates: [CP_QUEST]{"id":"event-id","questId":"stable quest id","title":"...","description":"...","objective":"...","status":"active/completed/failed (choose ONE)"}[/CP_QUEST]. Also provide issuer, objectives:[{id:"stable-step",text:"specific task",done:false}], and established rewards:{amount:100,xp:25,items:[]} when accepting a contract. Update completed objective flags as actions happen. All objectives must be done before status completed. Completion credits the player ONCE from contract funds, grants XP and loot automatically; do not emit duplicate INCOME/PROGRESS/LOOT rewards. Optional location:{"district":"watson","subdistrict":"Kabuki"} adds a mission marker. Location accepts the same named fields and established numeric x,y,z; omit unknown coordinates. Set location:null to remove its marker.
