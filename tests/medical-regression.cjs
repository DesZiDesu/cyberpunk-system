...
(async()=>{
 ...
 test('Blackwall frame mounts layered fractures without any side-screen labels',()=>{const fx=q('#cps-neural-fx');assert.equal(fx.textContent,'');assert.equal(fx.querySelectorAll('.cps-nfx-fracture').length,2);assert.equal(fx.querySelectorAll('.cps-nfx-rail').length,2);assert.equal(fx.querySelectorAll('.cps-nfx-crown').length,2);assert.equal(fx.querySelectorAll('.cps-nfx-tear').length,5);assert.ok(fx.querySelector('.cps-nfx-veil'));assert.equal(fx.querySelector('.cps-nfx-code,.cps-nfx-holo'),null);});
