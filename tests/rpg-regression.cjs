...
(async()=>{
 ...
 test('Stale extension drawer is upgraded to the running version and Cyberware entry',()=>{assert.equal(q('#cyberpunk-system-settings .cps-version').textContent,'v3.14.0');assert.ok(q('#cps-open-cyberware'));});
 test('Manifest, runtime, drawer and package versions agree',()=>{const manifest=JSON.parse(fs.readFileSync(path.join(repo,'manifest.json'))),pkg=JSON.parse(fs.readFileSync(path.join(repo,'package.json')));assert.equal(manifest.version,w.CyberpunkSystem.version);assert.equal(pkg.version,manifest.version);assert.ok(manifest.js.endsWith('?v='+manifest.version));assert.ok(manifest.css.endsWith('?v='+manifest.version));assert.ok(fs.readFileSync(path.join(repo,'ui/settings.html'),'utf8').includes('v'+manifest.version));});
 test('Public repository layout keeps loadable entries out of the root',()=>{const manifest=JSON.parse(fs.readFileSync(path.join(repo,'manifest.json'))),entry=v=>v.split('?')[0];assert.equal(entry(manifest.js),'src/index.js');assert.equal(entry(manifest.css),'styles/style.css');assert.ok(fs.statSync(path.join(repo,entry(manifest.js))).isFile());assert.ok(fs.statSync(path.join(repo,entry(manifest.css))).isFile());assert.deepEqual(fs.readdirSync(repo).filter(v=>/\.(?:js|css|html)$/.test(v)),[]);});
 ...
 test('Header role labels render but never become sparse NPC contacts',()=>{assert.ok(roleOnly.t.textContent.includes('Clouds Receptionist'));assert.equal(w.CyberpunkSystem.getNpcs().some(n=>n.name==='Clouds Receptionist'),false);});
 const completeNpc={id:'npc-profile-nika',name:'Nika Sato',handle:'nika_s',role:'Clouds receptionist',status:'Available',affiliation:'Clouds',age:'26',gender:'Woman',personality:'Observant and diplomatic; protects coworkers while quietly seeking a safer future.',appearance:'Short black hair, violet eye makeup, a Clouds uniform and a visible interface port.',notes:'Met Mael at Clouds and offered directions after checking the appointment.'};
 const profiled=await message(record('NPC_PROFILE',completeNpc)+'[CP_HEADER|Nika Sato|Clouds receptionist|Available][/CP_HEADER][CP_DIALOGUE|Nika Sato]Your appointment is ready.[/CP_DIALOGUE]');
 test('A new speaking NPC is stored only from a complete named dossier',()=>{const npc=w.CyberpunkSystem.getNpcs().find(n=>n.name==='Nika Sato');assert.ok(npc);for(const k of ['handle','role','status','affiliation','age','gender','personality','appearance','notes'])assert.ok(npc[k],k);assert.ok(!profiled.t.textContent.includes('CP_NPC_PROFILE'));});
 ctx.chatMetadata.cyberpunk_system.npcs=ctx.chatMetadata.cyberpunk_system.npcs.filter(n=>n.name!=='Nika Sato');w.CyberpunkSystem.refreshPrompt();
 ...
 test('Second Wand entry opens the player workspace',()=>{click('#cyberpunk-cyberware-wand');assert.ok(q('.cps-rpg-main'));assert.equal(d.querySelectorAll('.cps-rpg-tabs button').length,16);});
 ...
 test('Connecting reaches 100 percent and reveals complete shard text',()=>{assert.equal(q('.cps-shard-progress').getAttribute('aria-valuenow'),'100');assert.ok(q('.cps-shard-content').textContent.includes('east service elevator'));assert.ok(q('.cps-shard-content').textContent.includes('stairwell C'));});
 test('Field Ops document shows its title once and retains the supplied section structure',()=>{const reader=q('.cps-field-document');assert.equal(reader.querySelector('.cps-rpg-top h2').textContent,'Document');assert.equal(reader.querySelector('.cps-document-heading h3').textContent,'Konpeki service route');assert.equal(reader.textContent.split('Konpeki service route').length-1,1);assert.ok(reader.querySelector('.cps-shard-content').textContent.includes('Fallback'));assert.equal(reader.querySelector('.cps-document-section-index').textContent,'01');assert.equal(reader.querySelector('.cps-shard-reader').classList.contains('connecting'),false);});
 test('Read-state indicator is compact and document body no longer has its own scrolling box',()=>{const ast=require('postcss').parse(fs.readFileSync(path.join(repo,'styles/style.css'),'utf8'));let body=false,progress=false;ast.walkRules('.cps-ui.cps-field-document .cps-shard-content',r=>{assert.ok(r.nodes.some(n=>n.prop==='max-height'&&n.value==='none'));assert.ok(r.nodes.some(n=>n.prop==='overflow'&&n.value==='visible'));body=true;});ast.walkRules('.cps-ui.cps-field-document .cps-shard-link.complete .cps-shard-progress',r=>{assert.ok(r.nodes.some(n=>n.prop==='display'&&n.value==='none'));progress=true;});assert.ok(body&&progress);});
 click('[data-rpg=shard-close]');
 test('Closing a read shard archives exactly one persistent inventory item',()=>{const items=state().player.inventory.filter(x=>x.shardId==='konpeki-route');assert.equal(items.length,1);assert.equal(items[0].category,'data');assert.equal(state().shards[0].acquired,true);});
 click('[data-rpg="tab:inventory"]');click('[data-rpg="open-shard:konpeki-route"]');
 test('Archived shard can be reopened from Inventory without reconnecting',()=>{assert.equal(d.querySelector('[data-rpg=shard-connect]'),null);assert.ok(q('.cps-shard-content').textContent.includes('rotates at midnight'));});
 click('[data-rpg=shard-close]');click('[data-rpg="tab:quests"]');
 const offer=await message(record('QUEST_OFFER',{id:'offer-event-1',offerId:'nika-rescue',issuer:'Nika Sato',title:'Quiet Extraction',description:'Escort a coworker out through the service level.',objective:'Meet Nika at the service lift',objectives:[{id:'meet',text:'Meet Nika at the service lift'},{id:'escort',text:'Escort the coworker to safety'}],rewards:{amount:450,xp:80,items:[]},available:true}));
 test('A role-play mission offer opens decision UI and appears in Main Chat',()=>{assert.ok(offer.t.querySelector('[data-quest-offer="nika-rescue"]'));assert.ok(q('.cps-quest-offer-window'));assert.equal(state().quests.some(x=>x.id==='chat-gig:nika-rescue'),false);});
 click('.cps-quest-offer-window [data-offer-action=decline]');
 test('Declining does not create a mission and preserves a reconsiderable offer',()=>{assert.equal(state().questOffers.find(x=>x.id==='nika-rescue').status,'declined');assert.equal(state().quests.some(x=>x.id==='chat-gig:nika-rescue'),false);});
 offer.t.querySelector('[data-offer-action=accept]').click();
 test('A declined offer can be accepted later and creates one active mission',()=>{assert.equal(state().questOffers.find(x=>x.id==='nika-rescue').status,'accepted');const quests=state().quests.filter(x=>x.id==='chat-gig:nika-rescue');assert.equal(quests.length,1);assert.equal(quests[0].status,'active');assert.equal(quests[0].rewards.amount,450);});
