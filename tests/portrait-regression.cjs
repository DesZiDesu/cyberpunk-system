...
(async()=>{
 ...
 test('Cancelling the editor restores its manager',()=>{assert.ok(q('.cps-overlay').open);assert.equal(d.querySelector('.cps-modal'),null);});
 click('[data-record-add]');
 ...
 const picker=q('[data-portrait-file]');
 let blobCreations=0;const makeBlob=w.URL.createObjectURL;w.URL.createObjectURL=file=>{blobCreations++;return makeBlob(file);};
 const oversized=Buffer.alloc(24);oversized.writeUInt32BE(0x89504e47,0);oversized.writeUInt32BE(8000,16);oversized.writeUInt32BE(6000,20);
 Object.defineProperty(picker,'files',{configurable:true,value:[{type:'image/png',size:1000,slice:()=>({arrayBuffer:async()=>oversized.buffer.slice(oversized.byteOffset,oversized.byteOffset+oversized.length)})}]});picker.dispatchEvent(new w.Event('change',{bubbles:true}));
 await wait(()=>!picker.disabled);
 test('48-megapixel image is rejected before allocating a decoder or blob URL',()=>{assert.equal(blobCreations,0);assert.ok(q('.cps-studio-status').textContent.includes('24 megapixels'));assert.equal(q('[type="submit"]').disabled,false);});
 Object.defineProperty(picker,'files',{configurable:true,value:[{type:'image/png',size:bytes.length,bytes,slice:()=>({arrayBuffer:async()=>bytes.buffer.slice(bytes.byteOffset,bytes.byteOffset+bytes.length)})}]});
 picker.dispatchEvent(new w.Event('change',{bubbles:true}));
 test('Native file selection locks further uploads while decoding',()=>{assert.equal(picker.disabled,true);assert.equal(q('[type="submit"]').disabled,true);});
 await wait(()=>q('canvas').dataset.hasImage==='true'&&!q('[data-npc-generate]').disabled);
 test('Selected portrait decodes, releases its blob URL and enables a square crop',()=>{assert.equal(blobs.size,0);assert.equal(q('canvas').width,384);assert.equal(q('canvas').height,384);});
 ...
 await wait(()=>!q('[data-npc-generate]').disabled);
