  ...
  globalThis.CyberpunkDevicesFactory=api=>{
    ...
    function receive(data,source='',options={}){
      ...
      const entry={...old,id,name:C.text(data.name,180),type:data.type,description:C.text(data.description??old?.description,1000),
        ...
        data:C.text(data.data??old?.data,3000),effects:old?.effects||[],lastSeen:api.state().turn,source,recordId:C.text(data.id,160)};
    ...
    function decorate(element){
      ...
      const hasDevice=(root,id)=>[...root.querySelectorAll('[data-cps-device]')].some(el=>lookup(el.dataset.cpsDevice)?.id===id);
      if(current){
        const missing=b.devices.filter(d=>d.source===index+':swipe:'+(chat[index]?.swipe_id||0)&&d.location===b.location&&d.visit===b.visit&&!hasDevice(element,d.id));
        ...
        if(missing.length||wanted&&!element.querySelector('[data-cps-device]')){
          if(!tray){tray=document.createElement('div');tray.className='cps-device-discovery';element.append(tray);}
          for(const d of missing)if(!hasDevice(tray,d.id))tray.insertAdjacentHTML('beforeend',inline(d.id,d.name));
          if(wanted&&!element.querySelector('[data-cps-device]')&&!tray.querySelector('[data-device-scan]')){const scan=document.createElement('button');scan.type='button';scan.className='cps-device-inline';scan.dataset.deviceScan=wanted.type;if(wanted.suggestedName)scan.dataset.deviceName=wanted.suggestedName;scan.textContent=tr('Inspect: ','ตรวจอุปกรณ์: ')+wanted.name;tray.append(scan);}
        }
      }
      element.querySelectorAll('[data-device-scan]').forEach(el=>el.onclick=()=>{if(owner===api.chatBucket()&&el.isConnected)scan(el.dataset.deviceScan,el.dataset.deviceName||'');});
      element.querySelectorAll('[data-cps-device]').forEach(el=>{
        if(el.closest('pre,code'))return;
        el.onclick=e=>{e.preventDefault();e.stopPropagation();if(owner!==api.chatBucket())return;open(el.dataset.cpsDevice,el.textContent);};
      });
    }
