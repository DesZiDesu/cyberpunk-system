...
(() => {
  ...
  globalThis.CyberpunkDevicesFactory=api=>{
    ...
    let window=null,shownId=null,deckPickSlot=null;
    const label=p=>p?tr(p.en,p.th):tr('Device effect','ผลต่ออุปกรณ์');
    const button=(text,action,extra='')=>`<button type="button" class="cps-button" data-device-action="${E(action)}" ${extra}>${C.arrowLabel(text,E)}</button>`;
    ...
    function normalize(data){
      ...
      v.type=({security_camera:'camera',surveillance_camera:'camera',cctv:'camera',กล้อง:'camera',กล้องวงจรปิด:'camera',security_door:'door',ประตู:'door',security_drone:'drone',โดรน:'drone',auto_turret:'turret',computer:'terminal',access_point:'terminal',screen:'device',display:'device',billboard:'device',advertising_display:'device'})[type]||type;
      ...
      v.operation=action(v.operation);if(Array.isArray(v.actions))v.actions=[...new Set(v.actions.map(action))];
    ...
    function receive(data,source='',options={}){
      ...
      if(data.actions!==undefined&&!Array.isArray(data.actions))throw Error('Unsupported device actions: expected an array');
      const rejectedActions=(data.actions||[]).filter(id=>!programs.some(p=>p.id===id&&p.types.includes(data.type)));
      if(rejectedActions.length){data.actions=data.actions.filter(id=>!rejectedActions.includes(id));if(!data.actions.length)throw Error('Unsupported device actions: '+rejectedActions.join(', '));}
      ...
      const entry={...old,id,name:C.text(data.name,180),type:data.type,description:C.text(data.description??old?.description,1000),
        ...
        actions:data.actions||old?.actions||programs.filter(p=>p.types.includes(data.type)).map(p=>p.id),rejectedActions,
    ...
    function decorate(element){
      ...
      let latest=-1;for(let i=chat.length-1;i>=0;i--)if(!chat[i]?.is_user&&!chat[i]?.is_system){latest=i;break;}
      const current=node&&index===latest&&!chat[index]?.is_user&&!chat[index]?.is_system&&b.enabled&&api.settings().enabled&&api.settings().hackingEnabled&&api.state().bd.status==='stopped'&&!api.state().bd.rendering;
      ...
      for(const el of element.querySelectorAll('[data-device-mention]'))if(!lookup(el.dataset.deviceMention)&&!list.some(d=>d.id===el.dataset.deviceMention||word(d.name)===word(el.textContent)))list.push({id:el.dataset.deviceMention,name:el.textContent,type:'device',network:tr('Confirm observation','ยืนยันอุปกรณ์'),state:'UNRECORDED'});
      let wanted=intent();
      const raw=C.text(chat[index]?.mes,50000);let sawRecord=false;
      raw.replace(/\[CP_DEVICE\]([\s\S]*?)\[\/CP_DEVICE\]/gi,(_,json)=>{sawRecord=true;try{const v=normalize(JSON.parse(json.trim()));if(C.text(v.name,180)&&types.includes(v.type)){const id=deviceKey(v.deviceId||v.name);if(!list.some(d=>d.id===id||word(d.name)===word(v.name)))list.push({id,name:C.text(v.name,180),type:v.type,network:C.text(v.networkName,180)||tr('Confirm observation','ยืนยันอุปกรณ์'),state:'UNRECORDED'});if(!lookup(id))wanted??={type:v.type,name:C.text(v.name,180),suggestedName:C.text(v.name,180)};}else wanted??={type:'device',name:tr('Incomplete device record','ข้อมูลอุปกรณ์ไม่ครบ'),suggestedName:''};}catch{wanted??={type:'device',name:tr('Repair device record','แก้ข้อมูลอุปกรณ์'),suggestedName:''};}return _;});
      if(sawRecord&&!list.length&&!wanted)wanted={type:'device',name:tr('Repair device record','แก้ข้อมูลอุปกรณ์'),suggestedName:''};
      ...
      const draw=()=>{
        ...
        tray.querySelectorAll('[data-cps-device]').forEach(el=>el.onclick=e=>{e.preventDefault();e.stopPropagation();if(owner!==api.chatBucket()||!tray.isConnected)return;const item=list.find(d=>d.id===el.dataset.cpsDevice);if(lookup(el.dataset.cpsDevice))open(el.dataset.cpsDevice,item?.name);else scan(item?.type||'device',item?.name||'');});
      };draw();
    }
    ...
    function refreshWindow(){
      ...
      if(lookup(id)?.rejectedActions?.length){const warning=document.createElement('p');warning.className='cps-device-warning';warning.textContent=tr('Unsupported suggestions ignored: ','ข้ามคำสั่งที่ไม่รองรับ: ')+lookup(id).rejectedActions.join(', ');host.prepend(warning);}if(requested){const notice=document.createElement('p');notice.className='cps-device-warning';notice.setAttribute('role','status');notice.textContent=tr('Story request: ','คำขอจากเรื่อง: ')+label(requested)+tr(' — not executed. Complete Breach if required, then press Upload.',' — ยังไม่ได้ทำงาน เจาะระบบก่อนหากจำเป็น แล้วกดอัปโหลด');host.querySelector('.cps-device-network')?.before(notice);}
