...
globalThis.CyberpunkMailFactory = api => {
  ...
  function receive(data) {
    ...
    const doc={id,from,to:api.context()?.name1||'USER',subject,body,direction:'inbox',read:false,archived:false,pinned:false,at:new Date().toISOString(),threadId:C.text(data.threadId||id,160),replyTo:C.text(data.replyTo,160),offerId:terms?id:null};
    b.documents.push(doc);b.seen.push(id);if(terms)b.offers[id]=terms;
    changed();api.notify('NEW MAIL',from+' · '+subject);render();badge();return true;
  }
  ...
  function send(data) {
    ...
    folder='sent';selected=id;changed();render();return doc;
  }
  ...
  function render(){
    ...
    host.querySelectorAll('[data-mail]').forEach(b=>{if(b.closest('[data-mail-id]'))return;b.onclick=()=>safe(()=>{if(!live())return;const action=b.dataset.mail;
