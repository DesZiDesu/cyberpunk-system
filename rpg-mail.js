...
globalThis.CyberpunkMailFactory = api => {
  ...
  function editDocument(doc){
    ...
    editor.querySelector('form').onsubmit=e=>{e.preventDefault();try{
      ...
      Object.assign(doc,{subject,body,editedAt:C.uid()});changed();api.removeUiDialog(editor);render();
  ...
  async function requestReply(doc,regenerate=false) {
    ...
    try {
      ...
      if(regenerate){const subject=C.text(data.subject,180),body=C.text(data.body,20000);if(!subject||!body)throw Error('Generated mail text is missing');Object.assign(doc,{subject,body,editedAt:C.uid()});changed();render();}else receive({...data,threadId:doc.threadId,replyTo:doc.id});
